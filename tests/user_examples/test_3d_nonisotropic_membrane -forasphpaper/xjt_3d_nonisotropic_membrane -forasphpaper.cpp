...
//------------------------------------------------------------------------------
Real PL =  2.5; // 2.5e-3 ;	/**< X-direction domain. */  // non-10 
Real PH = 0.125;//e-3; /**< Z-direction domain. */ // non-0.125
Real BC = 0.3 * PL;
...
//----------------------------------------------------------------------
Real rho0_s = 2.0e-3; //2.0e3;   //non-2.0e-3; 
Real poisson = 0.26316; /**< Poisson ratio. */
Real Youngs_modulus = 8.242e6; //non-8.242e6;

Real diffusivity_constant_ = 1.0e-4; // 1.0e-10; //non-1.0e-4
Real fluid_initial_density_ = 1.0e-3; //1.0e3; //  non-1.0e-3;
...
class SaturationInitialCondition : public multi_species_continuum::PorousMediaSaturationDynamicsInitialCondition
{
  ...
  protected:
    void update(size_t index_i, Real dt = 0.0)
    {
		...
		Vecd center(0.5 * PL, 0.5 * PL, 0.0);
 
		Real r_square = pow((pos_[index_i][0] - center[0]), 2.0) + pow((pos_[index_i][1] - center[1]), 2.0) ;
		Real r_0_square = pow(0.5 * BC, 2.0);
		
			if (r_square  < r_0_square) 
		{
			Real  k_ =  1.0 - r_square / r_0_square;	
			fluid_saturation_[index_i] = k_ * full_saturation;			 
...
class TemperatureObserverParticleGenerator : public ObserverParticleGenerator
{
  public:
    ...
    {
        size_t number_of_observation_points = 6;
        Real range_of_measure = 1.0 * PH;
        Real start_of_measure = 0.0 * PH;

        for (size_t i = 0; i < number_of_observation_points; ++i)
        {
            Vec3d point_coordinate(0.5 * PL, 0.5 * PL, range_of_measure * (Real)i / (Real)(number_of_observation_points - 1) + start_of_measure);
...
int main()
{
 ...
 ObservedQuantityRecording<Real>
		write_beam_strain("VonMisesStrain", io_environment, my_observer_contact);
 ...
 ReducedQuantityRecording<ReduceDynamics<TotalMechanicalEnergy>>
    ...
    write_beam_strain.writeToFile(0);
	...
	while (GlobalStaticVariables::physical_time_ < End_Time)
	{
		...
		while (integration_time < D_Time)
		{
			...
			{
				if (total_kinetic_energy > (2e-4 * water_pressure_constant_))  // not here, total mechanicla neargy need to chnage to 2000
				{
			...
			write_beam_strain.writeToFile(ite);
