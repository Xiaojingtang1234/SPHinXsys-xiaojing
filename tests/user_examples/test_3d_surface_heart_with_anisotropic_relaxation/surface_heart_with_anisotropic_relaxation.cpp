...
class NonisotropicKernelCorrectionMatrixComplexAC : public LocalDynamics, public LaplacianSolidDataInner
{
  ...
  protected:
    ...
    void interaction(size_t index_i, Real dt = 0.0)
    {
       ...
       A1_[index_i] +=  r_ib[0] * r_ib[0] 
                            * (B_[index_i].transpose() * compensation_particle_dw_[index_i]);
             
       A2_[index_i] +=  r_ib[1] * r_ib[1] 
                            * (B_[index_i].transpose() * compensation_particle_dw_[index_i]);
                              
       A3_[index_i] +=  r_ib[2] * r_ib[2] 
                            * (B_[index_i].transpose() * compensation_particle_dw_[index_i]);

                           
       A4_[index_i] +=  r_ib[0] * r_ib[1] 
                            * (B_[index_i].transpose() * compensation_particle_dw_[index_i]);
                              
       A5_[index_i] +=  r_ib[1] * r_ib[2] 
                            * (B_[index_i].transpose() * compensation_particle_dw_[index_i]);
       
       A6_[index_i] +=  r_ib[0] * r_ib[2] 
                            * (B_[index_i].transpose() * compensation_particle_dw_[index_i]);
      ...
      public LaplacianSolidDataInner
{
  protected:
   typedef  ElectroPhysiologyParticlesforLaplacian::DiffusionReactionMaterial Material;
    ...
    StdLargeVec<Vec6d> G_, Laplacian_; 
  ...
  public: 
     ...
     void interaction(size_t index_i, Real dt = 0.0)
    {
        ...
        Vec6d S_compen = Vec6d(r_ib[0] * r_ib[0], r_ib[1] * r_ib[1], r_ib[2] * r_ib[2],
                             r_ib[0] * r_ib[1], r_ib[1] * r_ib[2], r_ib[2] * r_ib[0]);
    ...
    : public electro_physiology::ElectroPhysiologyInitialCondition
{
 ...
 class DiffusionGradientCheck : public LocalDynamics, public LaplacianSolidDataInner
{
  public:
    DiffusionGradientCheck(BaseInnerRelation &inner_relation)
        : LocalDynamics(inner_relation.getSPHBody()), LaplacianSolidDataInner(inner_relation), 
          B_(particles_->B_), pos_(particles_->pos_), 
         compensation_particle_dw_(particles_->compensation_particle_dw_),
         compensation_particle_rib_(particles_->compensation_particle_rib_)
    ...
    virtual ~DiffusionGradientCheck(){};
...
int main(int ac, char *av[])
{
    ...
    if (sph_system.RunParticleRelaxation())
    {
       ...
       /** A  Physics relaxation step. */
        relax_dynamics::RelaxationStepInner relaxation_step_inner(herat_model_inner, true);
 ...
 //   physiology_heart.defineAdaptationRatios(1.3, 1.0);
    ...
    Dynamics1Level<DiffusionGradientCheck> gradient_check(physiology_heart_inner);
